...
def add_tool(tool_json_str):
    try:
        ...
        name = tool["name"]
        DYNAMIC_TOOL_SCHEMAS[name] = tool
...
Your primary job is to answer the user's question clearly and helpfully.
You MAY have access to external tools, but ONLY the tools explicitly listed below.
-----------------------
GENERAL BEHAVIOR
-----------------------
- Always answer the user.
- If the question can be answered directly, respond normally in plain text.
- Greetings, explanations, and clarifications are allowed.
-----------------------
WHEN TO USE TOOLS
-----------------------
- Use a tool ONLY if the user's request explicitly requires external data
  or an action that cannot be answered with reasoning alone.
- If no suitable tool is available, answer in normal text.
...
- Arguments must match the tool parameters exactly
- Do NOT include explanations outside JSON

If a tool is NOT required:
...
"""
    ...
    tool_descriptions = []
    for tool in DYNAMIC_TOOL_SCHEMAS.values():
...
def try_parse_tool_call(text):
    ...
    # 🔒 HARD VALIDATION
    tool_name = tool_call.get("name")
    if tool_name not in DYNAMIC_TOOL_SCHEMAS:
        debug_log("INVALID TOOL NAME (REJECTED)", tool_name)
        return None

    debug_log("TOOL CALL DETECTED", tool_call)
    return tool_call
...
"""
    ...
    client.start_stream(callback=callback)  # ✅ REQUIRED
...
with gr.Blocks(title="LLM Playground vNext") as demo:
    gr.Markdown("## 🔥 LLM Playground (Dynamic Function Calling – Strict Mode)")
    ...
    gr.Markdown("### 🔧 Add Tool (Fireworks-style JSON)")
